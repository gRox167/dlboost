...
class LayerNorm3d(nn.LayerNorm):
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        var = torch.mean(centered**2, dim=1, keepdim=True)
        normalized = centered / torch.sqrt(var + self.eps)

        if self.elementwise_affine:
            # Apply scale and shift parameters
            normalized = normalized * self.weight.view(1, -1, 1, 1, 1) + self.bias.view(
                1, -1, 1, 1, 1
            )
...
def build_norm(name="ln3d", num_features=None, **kwargs) -> Optional[nn.Module]:
