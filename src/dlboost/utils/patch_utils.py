...
def _pad_for_scaning_windows(
    input: Tensor,
    patch_dims: Sequence[int] | NoneType,
    patch_size: Sequence[int] | NoneType,
    overlap: Sequence[float] | NoneType,
) -> Tuple[Tensor, PadSizes | None, torch.Size]:
    if patch_dims is None and patch_size is None and overlap is None:
        return input, None, input.shape
    else:
        pad_sizes = [(round(s * o),) * 2 for s, o in zip(patch_size, overlap)]
        output = pad(input, patch_dims, pad_sizes, mode="constant", value=0)
...
def _forward_for_scanning_windows(
    ...
    **kwargs: Any,
    ...
    for loc in patch_location:
        data = tree_map(lambda x: x.isel(loc, missing_dims="warn"), input_padded)
        ...
        output_isel = tree_map(lambda x: x.isel(loc, missing_dims="warn"), output)
...
def generate_patch_location(
    ...
    return tuple(
        slice(i, i + patch_size) for i in range(0, dimension_size - step+1, step)
    )
...
def infer(
    ...
    split_func: Callable[
        [torch.Tensor, Sequence[int], Sequence[int], Sequence[float]],
    ...
    **kwargs,
    ...
    input_splitted = split_func(input_padded, patch_dims, patch_size, overlap)

    output = _forward_for_scanning_windows(
        input_splitted,
        func,
        torch.zeros(padded_size, device=storage_device, dtype=input.dtype),
        lambda x: filter_func(x, patch_dims, patch_size, overlap),
        device,
        storage_device,
        *args,
        **kwargs,
...
def merge_dicts(
    dicts: PyTree[Dict], is_leaf=lambda x: is_bearable(x, NamedSize)
) -> Dict:
    return dict(ChainMap(*(m for m in tree_flatten(dicts, is_leaf=is_leaf)[0])))
...
def get_dim_size_dict_from_tree(
    ...
    """
    input_sizes = tree_map(lambda x: x.sizes, input)
    all_dims: Dict[str, int] = merge_dicts(
        input_sizes, is_leaf=lambda x: is_bearable(x, NamedSize)
    )

    return {d: all_dims[d] for d in dim_names}

def find_common_slices(
        data: DataArray, slices: Dict[str, slice]
):
...
def infer(
    ...
    storage_device: XArrayDevice = "xarray",
    ...
    **kwargs,
    ...
    pad_sizes = merge_dicts(pad_sizes_tree)
    ...
    output_padded_size_tree = tree_map(
        ...
        output_size_tree,
        is_leaf=lambda x: is_bearable(x, NamedSize),
    )

    output_padded_tree = tree_map(
        lambda x: DataArray(data = np.zeros(tuple(x.values())), dims = x.keys()),
        output_padded_size_tree,
        is_leaf=lambda x: is_bearable(x, NamedSize),
    )

    for loc in patch_location:
        data = tree_map(lambda x: x.isel(loc, missing_dims="warn"), input_padded_tree)
        ...
        output_padded_tree = tree_map_(lambda x, y: inplace_add_at_location(x, loc, y), output_padded_tree, result_filtered)
...
@overload
def cutoff_filter(data: PyTree[DataArray, "T"],
                  patch_size: Dict[str, int],
                    overlap: Dict[str, float]) -> PyTree[DataArray, "T"]:
    data = tree_map(lambda x: x.copy(deep=True), data)
    front_cut_slices = tree_map(lambda x, y: slice(0, int(x *  y)), patch_size, overlap)
    back_cut_slices = tree_map(lambda x, y: slice(int(x * (1-y)), None), patch_size, overlap)
...
# @overload
# def cutoff_filter(
#     x: PyTree[DataArray, "T"],
#     patch_size: Dict[str, int],
#     overlap: Dict[str, float],
# ) -> PyTree[DataArray, "T"]:
#     return tree_map(lambda x: cutoff_filter(x, patch_size, overlap), x)
...
def split_tensor(
    data: Tensor,
    dims: Sequence[int] | NoneType,
    patch_size: Sequence[int] | NoneType,
    overlap: Sequence[float] | NoneType,
) -> Sequence[Data_With_Location]:
    if dims is None and patch_size is None and overlap is None:
        return [data, (slice(None),) * data.dim()]
    for d, p, o in zip(dims, patch_size, overlap):
