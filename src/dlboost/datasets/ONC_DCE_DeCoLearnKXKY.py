...
def recon_one_scan(dat_file_to_recon, phase_num=10, time_per_contrast=10):
    reconstructor = recon.CAPTURE_VarW_NQM_DCE_PostInj(
        dat_file_location=dat_file_to_recon, phase_num=phase_num, time_per_contrast=time_per_contrast, which_slice=-1, which_contra=-1, which_phase=-1, device=torch.device('cuda:0'))
...
class ONC_DCE_DeCoLearn(LightningDataModule):
    def __init__(
        ...
        self.cache_dir = self.data_dir / '.cache_ONC_DCE_DeCoLearnKXKY_10ph'
    ...
    def setup(self, init=False, stage: str = 'train'):
        if stage == 'train' or stage == 'continue':
            ...
            # data = [zarr.open(self.cache_dir/"train"/(k+".zarr"), mode='r') for k in self.keys]
