...
class Recon(pl.LightningModule):
    def __init__(
        ...
        **kwargs,
        ...
        self.save_hyperparameters(
            ignore=['recon_module', 'cse_module', 'regis_module', 'recon_loss_fn', 'loss_fn'])
        ...
        if self.global_step % 4 == 0:
            for ch in [0, 3, 5]:
                to_png(self.trainer.default_root_dir+f'/image_init_moved_ch{ch}.png',
                       image_init_moved_ch[0, ch, :, :])  # , vmin=0, vmax=2)
                to_png(self.trainer.default_root_dir+f'/csm_moved_ch{ch}.png',
                       csm_moved[0, ch, :, :])  # , vmin=0, vmax=2)
    ...
    def cse_forward(self, image_init_ch):
        ...
        csm_lr = rearrange(
            csm_lr, '() (ph ch) h w -> ph ch h w', ph=ph)[:, :ch]
        csm_hr = csm_lr
        for i in range(3):
    ...
    def calculate_recon_loss(self, image_recon, csm, kspace_traj, kspace_data, weight=None):
        ...
        loss_not_reduced = \
            self.recon_loss_fn(torch.view_as_real(
                kspace_recon), torch.view_as_real(kspace_HHT))
