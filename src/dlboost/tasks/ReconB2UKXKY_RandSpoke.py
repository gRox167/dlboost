...
class Recon(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        # weight = torch.arange(-kspace_data.shape[-1]//2, kspace_data.shape[-1]//2, device=kspace_data.device)
        loss_revisit = (torch.view_as_real(kspace_data_estimated_blind*weight) + \
            lambda_ * torch.view_as_real(kspace_data_estimated_unblind*weight)
        - (lambda_+1) * torch.view_as_real(kspace_data*weight))**2
