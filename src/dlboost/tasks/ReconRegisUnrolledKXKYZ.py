...
class ReconRegis(pl.LightningModule):
    ...
    def forward_unrolled_red(self, batch):
        ...
        image_init = self.nufft_adj_fn(
            y*kspace_density_compensation, omega)
        ...
        for _ in range(self.unrolling_steps):
            ...
            prior = real_2ch_as_complex(sliding_window_inference(
    ...
    def training_step(self, batch, batch_idx):
        ...
        kspace_density_compensation_fixed = (
            batch['kspace_density_compensation_fixed']/3.5e-6)[:, None, None, :, :]
        # print(kspace_density_compensation_fixed.mean())
        kspace_density_compensation_moved = (batch['kspace_density_compensation_moved']/3.5e-6)[
        ...
        image_fixed = self.nufft_adj_fn(
            ...
            kspace_data_fixed*kspace_density_compensation_fixed, kspace_traj_fixed)
        image_moved = self.nufft_adj_fn(
            # kspace_data_moved, kspace_traj_moved)
            kspace_data_moved*kspace_density_compensation_moved, kspace_traj_moved)
        ...
        # regis_opt.step()
        cross_loss = 0
        # TODO nufft(nufft_adj(x)) is not equal to x, have 2/5 smaller std.
        loss_m2f = \
            self.__training_step_recon(image_recon=local_patch_fixed,
                                       ...
                                       image=image_moved)
        loss_f2m = \
            self.__training_step_recon(image_recon=local_patch_moved,
        ...
        self.manual_backward(
            cross_loss+self.loss_recon_consensus_COEFF*self_loss)
        recon_opt.step()
    ...
    def validation_step(self, batch, batch_idx):
        ...
        #     print(k,v.shape)
        for kspace_data_ch,cse_ch, kspace_traj_ch, kspace_density_compensation_ch in zip(batch["kspace_data"], batch["cse"], batch["kspace_traj"], batch["kspace_density_compensation"]):
            recon, init = self.forward_unrolled_red(
                dict(kspace_data=kspace_data_ch[None,...], kspace_traj=kspace_traj_ch[None,...], kspace_density_compensation=kspace_density_compensation_ch[None,None,...], cse=cse_ch[None,...])
            )
    ...
    def nufft(self, image, omega):
        ...
        # pdb.set_trace()
        image_kx_ky_z = self.nufft_op(torch.squeeze(image, dim=1), eo.rearrange(
            torch.view_as_real(omega), "b sp len c -> b c (sp len)"), norm="ortho")
        image_kx_ky_z = eo.rearrange(
            image_kx_ky_z, "b z (sp len) -> b z sp len", sp=shape_dict["sp"], len=shape_dict["len"])
    ...
    def nufft_adj_fn(self, kdata, omega):
        ...
        image = self.nufft_adj(kdata, eo.rearrange(
            torch.view_as_real(omega), "b sp len c -> b c (sp len)"), norm="ortho")
