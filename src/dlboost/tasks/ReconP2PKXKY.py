...
class Recon(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        image_fixed = self.nufft_adj_fn( # change nufft_fn to receive ktraj as complex number
    ...
    def validation_step(self, batch, batch_idx):
        ...
        image_init_list = []
        image_recon_list = []
        for kd,traj in zip(torch.split(kspace_data_compensated,8,dim=0), torch.split(kspace_traj,8,dim=0)):
            image_init = self.nufft_adj_fn(kd.cuda(), traj.cuda())
            image_init_list.append(image_init)
            image_recon_list.append(real_2ch_as_complex(
            self.recon_module(complex_as_real_2ch(image_init))).cpu())
    ...
    def nufft(self, image, omega):
        ...
        return eo.rearrange(
            image_kx_ky_z, "(b ph) ch (sp len) -> b ch ph sp len", b=shape_dict["b"], ph = shape_dict["ph"], sp=shape_dict["sp"], len=shape_dict["len"])

    def nufft_adj_fn(self, kdata, omega):
        # image is b c ph x y (c=1) and type of image is complex64
        shape_dict = eo.parse_shape(omega, "b ph sp len")
        image = self.nufft_adj(eo.rearrange(kdata, "b ch ph sp len -> (b ph) ch (sp len)"),
             eo.rearrange(torch.view_as_real(omega), "b ph sp len c -> (b ph) c (sp len)"), norm="ortho")
        return eo.rearrange(image, "(b ph) ch x y -> b ch ph x y", b = shape_dict["b"], ph = shape_dict["ph"])
