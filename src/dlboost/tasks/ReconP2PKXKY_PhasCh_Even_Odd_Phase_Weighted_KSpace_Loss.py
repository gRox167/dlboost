...
class Recon(pl.LightningModule):
    ...
    def __training_step_recon(self, image_recon, kspace_traj, kspace_data, weight):
    ...
    def forward_ch(self, kspace_data_compensated, kspace_traj, cse):
        ...
        for kspace_data_ch,cse_ch in zip(kspace_data_compensated ,cse):
            ...
            ch+=1
        # zarr.save(f'tests/P2PKXKY_PhasCh_Even_Odd_Phase_Weighted_KSpace_Loss/image_recon_.zarr', image_recon.abs().numpy(force=True))
        # zarr.save(f'tests/P2PKXKY_PhasCh_Even_Odd_Phase_Weighted_KSpace_Loss/image_init_.zarr', image_init.abs().numpy(force=True))
        return image_recon, image_init
    ...
    def predict_step(self, batch: Any, batch_idx: int, dataloader_idx: int = 0) -> Any:
        # bs = 
        for b in range(batch['cse'].shape[0]):
            image_recon, image_init = self.forward_contrast(batch["kspace_data_compensated"][b],
                                        batch["kspace_traj"][b], batch["cse"][b])
