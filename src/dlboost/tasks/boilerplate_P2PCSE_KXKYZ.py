...
def forward_contrast(kspace_data, kspace_traj, kspace_data_cse, kspace_traj_cse, recon_module, cse_forward, nufft_adj, inference_device, storage_device, inferer=None):
    """
    kspace_data: [ph, ch, z, len]
    kspace_traj: [ph, 2, len]
    """
    with torch.no_grad():
        csm = nufft_adj(kspace_data_cse.to(inference_device),
                        kspace_traj_cse.to(inference_device))
        # input of cse_forward shape is [ph, ch, d, h, w]
        csm = cse_forward(csm).to(storage_device)
        image_init = formap(nufft_adj_gpu, 2, 1, 10)(kspace_data,csm, kspace_traj=kspace_traj, nufft_adj=nufft_adj, inference_device=inference_device, storage_device=storage_device)
        if inferer is None:
...
def gradient_loss(s, penalty='l2', reduction='mean'):
    if s.ndim != 4:
        raise RuntimeError(
            f"Expected input `s` to be an 4D tensor, but got {s.shape}")
...
class P2PCSE_KXKYZ(L.LightningModule):
    def __init__(
        ...
        **kwargs,
        ...
        self.inferer = PatchInferer(
            SlidingWindowSplitter(patch_size, overlap=(
                0.5, 0, 0), offset=(1, 0, 0)),
            batch_size=8,
    ...
    def training_step(self, batch, batch_idx):
        ...
        csm_fixed = self.cse_forward(
            image_init_fixed_ch).expand(5, -1, -1, -1, -1)
        ...
        if self.global_step % 4 == 0:
            for ch in [0, 3, 5]:
                to_png(self.trainer.default_root_dir+f'/image_init_moved_ch{ch}.png',
                       image_init_moved_ch[0, ch, 0, :, :])  # , vmin=0, vmax=2)
                to_png(self.trainer.default_root_dir+f'/csm_moved_ch{ch}.png',
                       csm_moved[0, ch, 0, :, :])  # , vmin=0, vmax=2)
            for i in range(image_init_moved.shape[0]):
                to_png(self.trainer.default_root_dir+f'/image_init_ph{i}.png',
                       image_init_moved[i, 0, :, :])  # , vmin=0, vmax=2)
    ...
    def calculate_recon_loss(self, image_recon, csm, kspace_traj, kspace_data, weight=None):
        ...
        kspace_data_estimated = self.nufft_forward(
            image_recon * csm, kspace_traj)
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0):
        for b in batch:
            ...
            zarr.save(self.trainer.default_root_dir +
                      ...
                      f'/epoch_{self.trainer.current_epoch}'+'/image_init.zarr', image_init[:, 35:45].abs().numpy(force=True))
            zarr.save(self.trainer.default_root_dir +
                      #   f'/epoch_{self.trainer.current_epoch}'+f'/image_recon.zarr', image_recon.abs().numpy(force=True))
                      f'/epoch_{self.trainer.current_epoch}'+'/image_recon.zarr', image_recon[:, 35:45].abs().numpy(force=True))
            zarr.save(self.trainer.default_root_dir +
                      #   f'/epoch_{self.trainer.current_epoch}'+f'/csm.zarr', csm.abs().numpy(force=True))
                      f'/epoch_{self.trainer.current_epoch}'+'/csm.zarr', csm[:, :, 35:45].abs().numpy(force=True))
            ...
            for ch in [0, 3, 5]:
                ...
                to_png(self.trainer.default_root_dir+f'/epoch_{self.trainer.current_epoch}'+f'/csm_moved_ch{ch}.png',
                       csm[0, ch, 40, :, :])  # , vmin=0, vmax=2)
